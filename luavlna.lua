...
local allowed_space_names = {spaceskip = true, xspaceskip = true, userskip = true}
...
local allowed_kern_names = {italiccorrection=true}
local allowed_kern_types = {}
local kern_subtypes = node.subtypes("kern")
for k,v in pairs(kern_subtypes) do
  if allowed_kern_names[v] then
    allowed_kern_types[k] = true
  end
end
...
-- test if the next node after single char is a space or kern
local function is_space_after_single(nextn)
  if nextn == nil then return false end
  if nextn.id == glue_id then
    return true
  end
  return false
end

local function is_kern_after_single(nextn)
  -- handle italic correction kerning before space
  if nextn == nil then return false end
  local after_next = nextn.next
  if after_next == nil then return false end
  if nextn.id == kern_id and allowed_kern_types[nextn.subtype] and after_next.id == glue_id then
    -- when font changes the italic correction kerning is inserted, so we need to match is as true too
    return true
  end
  return false
end
...
local is_unit = function(word)
  ...
  for _, prefix in pairs(presi) do
    local s, e = string.find(word, prefix)
    if s == 1 then
  ...
  while head do
    ...
    if skip ~= 1 and not in_math  then
      ...
      elseif space==true and id == glyph_id and is_alpha(getchar(head)) then -- a letter 
        ...
        if test_fn(char, s) then
          if is_space_after_single(nextn) then    -- only if we are at a one letter word
            head = insert_penalty(head)
          elseif is_kern_after_single(nextn) then
            -- if there is an italic correction after single letter, we should prevent spaces too
            single_kern = true
          end
        end
        space = false
